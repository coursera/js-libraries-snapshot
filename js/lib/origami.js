...
function($, _, Backbone, require, Modal, confirmNavigationTemplate) {
  ...
  var _private = {
    ...
    regionHasUnsavedModel: function(region) {
      ...
      var viewHasUnsavedModel =
        region.view.hasUnsavedModel
          ? region.view.hasUnsavedModel()
          : false;
  ...
  var router = Backbone.Router.extend({
    ...
    navigate: function(fragment, options) {
      if (_private.current &&
          _private.regionHasUnsavedModel(_private.current)) {
    ...
    this.region = {
      ...
      open: function(name, callback) {
        ...
        that.region.fetch(view, conf, callback || function(parent) {
          ...
          var validRegions = _(parent.regions).filter(function(region) { return region; });
