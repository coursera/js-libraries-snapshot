...
;(function() {
  ...
  var Modals = function($, LucidJS, _, DataAttributes) {
    ...
    var _private = {
      ...
      setOverlayHeight: function() {
        if ($(window)
          .height() < $(document)
          .height()) _private.$overlay.css({
          height: $(document)
            .height() + 'px'
        });
        else _private.$overlay.css({
          height: '100%'
        });
      },
      ...
      open: function() {
        ...
        if(this.$iframe) {
          ...
          var $focus = this.$el.find("[" + this.options["attribute.focus"] + "]").filter(":visible");
          var $focusable_elements = this.$el.find("a,input,select,textarea,button").filter(":visible");
          for(i=0;i<$focusable_elements.length;i++){
            $focusable_elements.eq(i).attr("tabindex",this.options["attribute.focus.index"]+1+i);
          }
          if ($focus.size() > 0) {
            $focus.eq(0).attr("tabindex",this.options["attribute.focus.index"]);
            $focus.eq(0).focus();
          } else if ($focusable_elements.size() > 0) {
            $focusable_elements.eq(0).attr("tabindex",this.options["attribute.focus.index"]);
            $focusable_elements.eq(0).focus();
          }
        }
        ...
        this.$el.on(this.options['bind.action'], '[' + this.options['attribute.action'] + ']', _.bind(_private.actionTrigger, this));
    ...
    Modal.prototype.open = function() {
      ...
      if(this.options['iframe.url'])
        iframeUrl = this.options['iframe.url'];
      ...
      if (this.options['overlay.show']) {
        if (_private.$overlay && _private.$overlay.css('display') != 'none' && _private.$overlay.hasClass(this.options['overlay.class'])) {
          this.$overlay = _private.$overlay;
        } else {
          if (_private.$overlay) _private.$overlay.off('.modal')
            .hide()
            .remove();

          this.$overlay = _private.$overlay = $('<div>')
            .addClass(this.options['overlay.class'])
            .hide();
          this.$overlay.css({
            position: 'fixed',
            width: '100%',
        ...
        if (this.options['animate.overlay.open']) this.$overlay.animate(this.options['animate.overlay.open'], {
      ...
      if(this.options['position'] != 'fixed' || this.$el.closest('html').length === 0)
      ...
      if (this.options['animate.modal.open']) {
        this.$el
          .animate(this.options['animate.modal.open'], {
      ...
      if (this.options['attribute.close']) {
        this.$el.on(this.options['bind.close'] + '.modal', '[' + this.options['attribute.close'] + ']', function(e) {
          ...
          if ($target.attr('disabled') != 'disabled') {
            ...
            if($target.attr(that.options["attribute.action"]) === undefined)
              that.emitter.trigger('action', false, that);
    ...
    Modal.prototype.close = function() {
      ...
      if(--_private.count === 0) { 
        if (this.options['overlay.show'] && this.$overlay) {
          ...
          if (this.options['animate.overlay.close']) this.$overlay.animate(this.options['animate.overlay.close'], {
            duration: this.options['animate.overlay.duration'],
            complete: function() {
    ...
    external.stop = function(view) {
      ...
      if (_private.prevModal)
        Modal(_private.prevModal).close();
    };
  ...
  if (typeof define === "function" && define.amd) {
    define(['jquery', 'js/lib/lucid', 'underscore', 'js/lib/data.attributes'], function($, LucidJS, _, DataAttributes) {
      return Modals($, LucidJS, _, DataAttributes);
    });
  } else if (typeof window !== 'undefined' && typeof ender === 'undefined') {
      window.Modal = Modals(window.$, window.LucidJS, window._, window.DataAttributes);
  }
